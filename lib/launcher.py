...
from solver import display_solved_picture, grid_valid
...
def solver(path, debug=False):
    ...
    if grid_valid(board):
        ...
        print("Grid is not valid. Error in construction")  
        if debug:
            plt.subplot(121), plt.imshow(IMG), plt.title('Main Grid detection')
            plt.xticks([]), plt.yticks([])
            plt.subplot(122), plt.imshow(PIC_PP, cmap='gray'), plt.title('Preprocessed grid')
            plt.xticks([]), plt.yticks([])
            plt.show()
